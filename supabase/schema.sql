...
create policy "Users can view their own habit event logs" on public.habit_event_logs for select using (auth.uid() = user_id);
create policy "Users can create their own habit event logs" on public.habit_event_logs for insert with check (auth.uid() = user_id);
create policy "Users can update their own habit event logs" on public.habit_event_logs for update using (auth.uid() = user_id);
create policy "Users can delete their own habit event logs" on public.habit_event_logs for delete using (auth.uid() = user_id);
