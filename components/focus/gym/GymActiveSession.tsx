...
export function GymActiveSession({ config }: { config: GymLayerConfig }) {
    ...
    // Derived overall active time (mocked for simplicity here, normally linked to focus session)
    ...
    const activeExercise = config.exercises.find(e => e.id === config.activeExerciseId) || config.exercises[0];
    ...
    return (
        <div className="w-full max-w-[400px] h-[750px] bg-black/40 backdrop-blur-3xl rounded-[40px] border border-white/[0.07] overflow-hidden flex flex-col relative shadow-[0_8px_40px_rgba(0,0,0,0.5)]">
            {/* Dynamic Glow Based on Workout Color */}
            <GlowingEffect
                spread={80}
                proximity={120}
                inactiveZone={0.01}
                ...
                disabled={false}
                customGradient={
                    `radial-gradient(circle, ${workoutColor} 10%, transparent 20%),
            ...
            <div className="px-6 pt-10 pb-4 flex justify-between items-center text-white sticky top-0 bg-transparent z-20">
                <div>
                    <h1 className="text-[22px] font-extrabold tracking-tight relative" style={{ color: workoutColor, textShadow: `0 0 20px ${workoutColor}80` }}>
                        {activeExercise?.name || 'Workout'}
                    </h1>
                </div>
            ...
            <div className="px-6 flex-1 overflow-y-auto scrollbar-none pb-32">
                ...
                <div className="flex justify-between items-center mb-6 px-1">
                    <div className="flex items-center gap-2 text-white/40 text-xs font-bold uppercase tracking-widest">
                        <Play className="w-3.5 h-3.5" />
                        {Math.floor((activeExercise?.programmedRest || 180) / 60)}:00 rest
                    </div>
                ...
                <div className="grid grid-cols-[3rem_1fr_1fr] gap-4 mb-2 px-2">
                    <div className="text-[10px] text-white/30 font-bold uppercase tracking-widest text-center">Set</div>
                    <div className="text-[10px] text-white/30 font-bold uppercase tracking-widest text-center">Reps</div>
                    <div className="text-[10px] text-white/30 font-bold uppercase tracking-widest text-center">Weight</div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                    {activeExercise?.sets.map((set, i) => (
                        ...
                        >
                            ...
                            >
                                {set.isCompleted ? <Check className="w-5 h-5" /> : (i + 1)}
                            </button>
                            <input
                                type="number"
                                value={set.reps || ''}
                                onChange={e => updateGymSet(activeExercise.id, set.id, { reps: parseInt(e.target.value) || 0 })}
                            ...
                            <div className="relative flex items-center justify-center">
                                <input
                                    type="number"
                                    value={set.weight || ''}
                                    onChange={e => updateGymSet(activeExercise.id, set.id, { weight: parseFloat(e.target.value) || 0 })}
            ...
            <div className="absolute bottom-6 left-6 right-6 z-10">
                <GlassButton
                    onClick={() => {
                        // Finish workout action (clear config logically)
                        setLayer(null);
                    }}
                    variant="gym"
