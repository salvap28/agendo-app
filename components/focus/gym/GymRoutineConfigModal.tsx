...
import { getWeekdayInitialsMondayFirst } from "@/lib/i18n/app";
import { getGymRoutineConfigCopy } from "@/lib/i18n/ui";
...
export function GymRoutineConfigModal({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
    ...
    const weekdayLabels = getWeekdayInitialsMondayFirst(language);
    const copy = getGymRoutineConfigCopy(language);
    ...
    const toggleDay = (idx: number) => {
        setPlannedDays((prev) => (prev.includes(idx) ? prev.filter((day) => day !== idx) : [...prev, idx]));
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-[400px] bg-black/60 backdrop-blur-3xl rounded-[40px] border border-white/[0.07] overflow-hidden flex flex-col shadow-[0_8px_40px_rgba(0,0,0,0.5)] relative">
                <div className="px-6 pt-6 pb-2 flex justify-between items-center text-white">
                    <div className="text-white/40 text-sm font-medium">{copy.newRoutine}</div>
                    <GlassButton
                ...
                <div className="px-6 pb-6 mt-2 flex-1 max-h-[70vh] overflow-y-auto scrollbar-none">
                    ...
                    <div className="bg-white/[0.02] border border-white/[0.05] rounded-3xl p-5 mb-4 group transition-all">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-white font-medium text-sm">{copy.color}</span>
                        </div>
                            ...
                            {COLORS.map((swatch) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        color === swatch ? "ring-2 ring-white ring-offset-2 ring-offset-[#1c1c1e]" : "",
                                    )}
                    ...
                    <div className="bg-white/[0.02] border border-white/[0.05] rounded-3xl p-5 mb-4 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            <span className="text-white font-medium text-sm">{copy.schedule}</span>
                        </div>
                        <div className="flex gap-1.5">
                            {weekdayLabels.map((dayLabel, index) => {
                                const active = plannedDays.includes(index);
                                return (
                                    <button
                                        key={`${dayLabel}-${index}`}
                                        ...
                                        className={cn(
                                            ...
                                            active ? "bg-white text-black" : "bg-white/5 text-white/40",
                                        )}
                                    >
                                        {dayLabel}
                                    </button>
                    ...
                    <div className="bg-white/[0.02] border border-white/[0.05] rounded-3xl p-5 mb-4 flex justify-between items-center">
                        <div className="flex flex-col">
                            <span className="text-white font-medium text-sm">{copy.restTimer}</span>
                        </div>
                        <select
                            value={restSec}
                    ...
                    <div className="bg-white/[0.02] border border-white/[0.05] rounded-3xl p-5 mb-8">
                        <div className="flex justify-between items-center mb-4">
                            <span className="text-white font-medium text-sm">{copy.exercises} ({exercises.length})</span>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 mb-4">
                            {exercises.map((exercise, index) => (
                                <div
                                    key={exercise.id}
                                    className="text-sm text-white/80 bg-white/[0.05] px-3 py-2 rounded-xl flex justify-between items-center border border-white/[0.05]"
                                >
                                    <span>{index + 1}. {exercise.name}</span>
                                    <button
                                        onClick={() => setExercises((prev) => prev.filter((item) => item.id !== exercise.id))}
                        ...
                        <input
                            type="text"
                            value={exInput}
                            onChange={(event) => setExInput(event.target.value)}
                            onKeyDown={(event) => {
                                if (event.key === "Enter" && exInput.trim()) {
                                    setExercises((prev) => [...prev, { id: crypto.randomUUID(), name: exInput.trim() }]);
                                    setExInput("");
                                }
                            }}
                            placeholder={copy.addExercise}
