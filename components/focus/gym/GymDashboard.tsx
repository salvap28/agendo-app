...
import { getGymDashboardCopy } from '@/lib/i18n/ui';
...
export function GymDashboard() {
    ...
    const copy = getGymDashboardCopy(language);
            ...
            <div className="px-6 pt-10 pb-6 flex-1 overflow-y-auto scrollbar-none relative z-10">
                <div className="flex justify-between items-center mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-2xl font-bold text-white/90 tracking-tight">{copy.routines}</h1>
                    </div>
                    <GlassButton
                ...
                <div className="w-full rounded-3xl p-5 mb-8 relative overflow-hidden border border-white/10 group cursor-pointer" onClick={() => setIsConfigOpen(true)}>
                    ...
                    <div className="relative z-10 flex justify-between items-center">
                        <div>
                            <h2 className="text-white font-extrabold text-xl mb-1">{copy.newRoutine}</h2>
                            <p className="text-white/60 text-xs">{copy.newRoutineBody}</p>
                        </div>
                ...
                {/* Workouts Grid */}
                <h3 className="text-white font-bold text-lg mb-4">{copy.workouts}</h3>

                {isLoading ? (
                    <div className="text-white/40 text-sm">{copy.loading}</div>
                ) : (
                        ...
                        {routines.map(routine => (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <span className="text-[11px] text-white/40 font-medium">{copy.exercisesCount(routine.exercises?.length || 0)}</span>
                                </div>
                                ...
                                <div className="flex items-end justify-between">
                                    <div className="flex gap-3">
                                        <div>
                                            ...
                                            <div className="text-[9px] text-white/40 uppercase tracking-widest mt-1">{copy.rest}</div>
                                        </div>
                                    </div>
